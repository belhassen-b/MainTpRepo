...
public class LikeMentionServiceImpl implements ILikeMentionService {
    ...
    public LikeMentionDto update(Long id, LikeMentionDto dislikeMentionDto) {
        LikeMention likeMention1 = likeRepository.findById(id).get();
    ...
    public List<LikeMentionDto> findAll() {
        ...
        return dislikeMentions.stream().map(dislikeMention -> modelMapper.map(dislikeMention, LikeMentionDto.class)).collect(java.util.stream.Collectors.toList());
    }
