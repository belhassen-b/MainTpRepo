...
public class TaskDAOImpl implements TaskDAO {
    ...
    public List<Task> getTasksByCategoryId(Long categoryId) {
        EntityManager entityManager = entityManagerFactory.createEntityManager();
        List tasks = entityManager.createQuery("SELECT t FROM Task t WHERE t.id = :categoryId")
    ...
    public boolean addTaskToCategory(Long taskId, Long categoryId) {
        ...
        transaction.begin();
        Task task = entityManager.find(Task.class, taskId);
        task.setCategory(entityManager.find(Category.class, categoryId));
        entityManager.persist(task);
        transaction.commit();
        entityManager.close();
        return true;
    ...
    public boolean deleteTaskFromCategory(Long taskId, Long categoryId) {
        EntityManager entityManager = entityManagerFactory.createEntityManager();
        EntityTransaction transaction = entityManager.getTransaction();
        transaction.begin();
        Task task = entityManager.find(Task.class, taskId);
