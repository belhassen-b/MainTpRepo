-- 1 Création de la base de données et de la table Livres


CREATE DATABASE IF NOT EXISTS exercice4 CHARACTER SET utf8 COLLATE utf8_general_ci;
...
-- 3 Requete permettant d'afficher 10  livres + anciens classés par ordre croissant de titre
SELECT * FROM livre ORDER BY date_publication  LIMIT 10;
SELECT * FROM (SELECT * FROM livre ORDER BY date_publication  LIMIT 10) AS livres_ancien ORDER BY titre ASC;

-- 4 10 livres + anciens ( colonne date pub +auteur + titre) classés par date de publication croissante
SELECT titre, auteur, date_publication FROM livre ORDER BY date_publication ASC LIMIT 10;

-- 5 Afficher tous les livres d'Agatha Christie
SELECT * FROM livre WHERE auteur = 'Agatha Christie';

-- 6 Requête pour corriger erreur
UPDATE livre SET auteur = 'Agatha Christie' WHERE auteur = 'Agatha Christies';
SELECT * FROM livre WHERE auteur = 'Agatha Christie';

-- 7 Inserer le livre de votre choix
INSERT INTO livre (titre, auteur, editeur, date_publication, isbn_10, isbn_13)
VALUES
('Le Petit Prince','Antoine de Saint-Exupéry','Gallimard','1943-04-06','2070412393','978-2070412396');

-- 8 Supprimer un livre par critère auteur et titre
DELETE FROM livre WHERE auteur = 'Agatha Christie' AND titre = 'Un cadavre dans la bibliothèque';